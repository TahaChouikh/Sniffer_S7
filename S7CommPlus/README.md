...
``` bash
sudo docker run -it   --entrypoint /bin/sh   --network host   --cap-add=NET_ADMIN   --cap-add=NET_RAW   s7commplus
```
...
``` bash
sudo docker run -it   --network host   --cap-add=NET_ADMIN   --cap-add=NET_RAW   s7commplus
```
